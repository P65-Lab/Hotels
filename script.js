...
function installerNouvelHotel(){

  const ville = document.getElementById("newVille");
  const villeResults = document.getElementById("newVilleResults");

  const hotel = document.getElementById("newHotel");
  const hotelResults = document.getElementById("newHotelResults");

  if(!ville || !villeResults || !hotel || !hotelResults){
    return;
  }
  ...
  function afficherVilles(){
    ...
    villeResults.innerHTML = liste.map(v => `
      <button
        type="button"
        class="search-result"
        data-ville="${v.replace(/"/g,"&quot;")}"
  ...
  ville.onclick = afficherVilles;
  ville.onfocus = afficherVilles;
  ...
  function afficherHotels(){
    ...
    hotelResults.innerHTML = liste.map(item => `
      <button
        type="button"
        class="search-result"
        data-hotel="${item.hotel.replace(/"/g,"&quot;")}"
      >
        ${item.hotel}
      </button>
    `).join("");
  ...
  hotel.onclick = afficherHotels;
  hotel.onfocus = afficherHotels;

}


installerNouvelHotel();
